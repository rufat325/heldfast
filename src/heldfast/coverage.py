...
def _registry(entry: dict | None, spec: Any, offline: bool = False) -> Layer:
    ...
    if recorded:
        ...
        return Layer("registry pin", "?",
                     ...
                     "heldfast scan compares it against the registry; a launch "
    ...
    if version and not _FLOATING.match(version):
        ...
        return Layer(
            ...
            "heldfast approve --probe records the tarball hash")
    return Layer(
        "registry pin", "no",
...
def _in_path(key: str, entry: dict | None, spec: Any, fronting: set) -> Layer:
    ...
    if spec is not None and spec.is_remote:
        ...
        return Layer(
            ...
            "point the client at `heldfast gateway`, which fronts hosted servers "
            "over HTTP. `heldfast guard` cannot: it wraps a child process and "
            "there is none")
    return Layer(
        "enforced", "no",
        "the client talks to this server directly; nothing checks the lockfile at runtime",
        "point the client at `heldfast gateway`, or wrap it with `heldfast guard`")
