...
The `anthropic` SDK is an optional extra (`pip install heldfast[llm]`);
...
def _load_client(api_key: str | None = None):
    ...
    try:
        ...
        raise LLMUnavailable(
            "the --llm tier needs the anthropic SDK, which heldfast does not "
            "install by default.\n"
            "           Install it with:  pip install 'heldfast[llm]'"
        ) from None
    try:
