...
effects. `heldfast` scanning its own server is expected to come back clean,
...
# heldfast's own rules (MCPA021, MCPA022) read these claims on other servers;
...
def tool_check_config(args: dict[str, Any]) -> dict[str, Any]:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="heldfast-")) / ".mcp.json"
    try:
...
def tool_check_coverage(args: dict[str, Any]) -> dict[str, Any]:
    ...
    agent using heldfast as a server most obviously has about itself, and the
    ...
    if not lock.servers:
        payload["note"] = (
            ...
            "guarantees can be in force. Run `heldfast approve --probe` first."
        )
    return payload
