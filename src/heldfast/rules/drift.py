...
def unapproved_server(ctx: AuditContext) -> Iterable[Finding]:
    ...
    for s in ctx.servers:
        ...
        yield Finding(
            ...
            remediation=(
                "Review the server, then run `heldfast approve` to record it. An MCP server "
...
def tool_drift(ctx: AuditContext) -> Iterable[Finding]:
    ...
    if ctx.lock.get("stale_digests"):
        ...
        yield Finding(
            ...
            remediation=(
                "Re-run `heldfast approve` to record the current state with the new "
        ...
        for path, approved in unmatched(recorded, current):
            ...
            yield Finding(
                ...
                remediation=(
                    ...
                    "editing a config somebody committed. Re-run `heldfast approve "
...
def _integrity_finding(s: Any, key: str, detail: str, *,
    ...
    return Finding(
        ...
        remediation=(
            ...
            "it, then `heldfast approve --probe --yes`."
        ),
        server=s.identity(),
        atlas=["AML.T0010.001"],
        cwe=["CWE-494"],
    ...
    for s, recorded, urls in _recorded_integrity(ctx):
        if not recorded:
            ...
            yield Finding(
                ...
                remediation=(
                    "Re-run `heldfast approve` where the registry is reachable. "
