...
class Gateway:
        ...
        for spec in servers:
            ...
            if guard._locked_tools is None and not allow_unapproved:
                ...
                self.log(f"not started: {spec.identity()} is not in the lockfile. "
                         f"Run `heldfast approve --probe`, or pass "
                         f"--allow-unapproved.")
                continue
    ...
    def log(self, message: str) -> None:
        if not self.quiet:
            print(f"heldfast gateway: {message}", file=sys.stderr)
        ...
        for name, backend in self.backends.items():
            ...
            for tool in screened:
                ...
                if self.identity is not None and \
                    ...
                    continue
                if "BLOCKED BY heldfast" in str(tool.get("description") or ""):
    ...
    def handle_call(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(params, dict):
            return self._error(request_id, "[heldfast] malformed tools/call")
        ...
        if split is None:
            ...
            return self._error(request_id, (
                f"[BLOCKED BY heldfast] no approved server offers "
        ...
        if self.identity is not None:
            if self.identity.denies_tool(namespaced, tool):
                ...
                return self._error(request_id, (
                    f"[BLOCKED BY heldfast] {namespaced} is not available to "
                ...
                if not decision.allowed:
                    ...
                    if self.trail:
                        self.trail.record("denied", subject=namespaced,
                                          ...
                                          detail=f"identity={self.identity.name}")
                    return self._error(request_id, (
                        f"[BLOCKED BY heldfast] {namespaced} was not called. "
        ...
        if self.max_calls:
            ...
            if used >= self.max_calls:
                ...
                if not self.dry_run:
                    return self._error(request_id, (
                        f"[BLOCKED BY heldfast] {namespaced} has been called "
        ...
        if reply is None:
            return self._error(request_id, (
                f"[heldfast] {name} did not answer. The gateway is still up; "
...
def run(servers: list[ServerSpec], lock_path: Path, *, policy: str = "block",
    ...
    try:
        ...
        print(f"heldfast gateway: {exc}", file=sys.stderr)
        return 2
    ...
    if log_path is not None:
        ...
        if trail.failed:
            print(f"heldfast gateway: audit log unavailable: {trail.failed}",
                  file=sys.stderr)
            trail = None
        else:
    ...
    gateway.start()
    if not gateway.backends:
        print("heldfast gateway: no backend started successfully.", file=sys.stderr)
        gateway.close()
        return 2
