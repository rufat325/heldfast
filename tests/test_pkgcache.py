...
from fake_npm_cache import empty_npm_cache, fake_npm_cache, write_entry  # noqa: E402
...
class TestNpmCache(unittest.TestCase):
    ...
    def test_an_unscoped_package_resolves_too(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with fake_npm_cache(tmp, "leftpad", "1.0.0", "sha512-abc"):
                got = pkgcache.check({"npm:leftpad@1.0.0": "sha512-abc"})
        self.assertEqual(["verified"], [c.state for c in got])
    ...
    def test_the_last_entry_in_an_appended_index_wins(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            write_entry(root, "@scope/pkg", "1.2.3", "sha512-old")
            write_entry(root, "@scope/pkg", "1.2.3", "sha512-new")
            previous = os.environ.get("npm_config_cache")
            os.environ["npm_config_cache"] = str(root)
            try:
                got = pkgcache.check({"npm:@scope/pkg@1.2.3": "sha512-new"})
            finally:
                if previous is None:
    ...
    def test_a_corrupt_index_line_does_not_raise(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            path = write_entry(root, "@scope/pkg", "1.2.3", "sha512-abc")
