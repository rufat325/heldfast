...
`plugin.json`, the two npm manifests and the string `mcp-pin-check --version`
...
from mcp_pin import __version__  # noqa: E402
...
class TestEveryStatedVersionAgrees(unittest.TestCase):
    ...
    def test_the_plugin_manifest_agrees(self) -> None:
        manifest = json.loads(_read("plugin/mcp-pin/.claude-plugin/plugin.json"))
        self.assertEqual(__version__, manifest["version"])

    def test_both_npm_manifests_agree(self) -> None:
        for relative in ("js/mcp-pin-check/package.json",
                         "js/mcp-pin-wrap/package.json"):
    ...
    def test_the_checker_prints_the_version_it_is(self) -> None:
        ...
        printed = re.search(r'mcp-pin-check (\d+\.\d+\.\d+)',
                            _read("js/mcp-pin-check/bin.js"))
