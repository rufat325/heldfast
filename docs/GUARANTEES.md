...
| T-REDIRECT | A redirect to a different origin does not carry the request's credential headers, and a hop onto cleartext for a non-loopback host is refused outright. Following redirects is required by the transport -- including 307 and 308, which keep the method and body a POST-only transport depends on and which the stdlib refuses -- but handing `Authorization` to whatever a `Location:` names is not. | `tests/test_fetch.py` |
