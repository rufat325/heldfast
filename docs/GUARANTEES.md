...
| T-UNVERIFIED | An artifact that could not be checked is reported as MCPA037 and never as verified, and so is one the lockfile records no hash for at all -- the second is less evidence than the first, so it cannot be the quieter of the two. `--require-integrity` makes both high, and makes `guard` and `gateway` refuse to start. | `tests/test_pkgcache.py`, `tests/test_integrity.py`, `tests/test_attack_corpus.py`, `tests/test_mutation.py` |
...
| T-REDIRECT | A redirect to a different origin does not carry the request's credential headers. Following redirects is required by the transport; handing `Authorization` to whatever a `Location:` names is not. | `tests/test_fetch.py` |
