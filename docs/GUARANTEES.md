...
| T-DRIFT-GRADED | Without `--drift graded`, a tool whose definition changed is refused, as it always was; the heuristic is never the default. Under it, `guard` and `gateway` forward a changed tool only when the live definition -- description, title, and every description or title in its input and output schemas -- introduces no signal (an agent-directed instruction, a hidden character, a credential path, a look-alike letter, a critical word) that the approved preview did not carry. Text past what the lock recorded is compared against nothing, so a signal there refuses. A tool absent at approval is still refused, a schema too deep to read is not called clean, and a grading error refuses the tool even under `--fail-open`. The content rules still read every forwarded tool whole. What a rewrite phrased to miss every pattern can do is best-effort, below, not this. | `tests/test_drift_graded.py`, `tests/test_mutation.py` |
