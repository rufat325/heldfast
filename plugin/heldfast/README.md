...
**Graded drift.** Set `MCP_PIN_DRIFT=graded` and a changed tool is allowed when the change introduced no signal -- no new agent-directed instruction, hidden character, credential path or look-alike letter -- and denied, with what it gained, when it did. The hook asks `heldfast grade-drift` (the same test `wrap --drift graded` runs), so it needs `heldfast` on `PATH`, or `MCP_PIN_PYTHON` pointing at an interpreter that has it. If the grader cannot run, the call is denied. It is a heuristic: see [MANUAL.md](../../docs/MANUAL.md#upgrades-without-the-re-approval-treadmill---drift-graded).
